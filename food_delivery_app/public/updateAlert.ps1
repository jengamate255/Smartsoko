$lines = @(
  ...
  '    alertElement.className = "bg-white dark:bg-gray-800 border rounded-lg shadow-lg p-4 mb-2 transform: translateX(100%) opacity: 0 transition-all 0.3s ease border-left-width-4 border-left-style-solid";',
  '',
  ...
  '          <span class=\"material-symbols-outlined text-lg ', alert.type === 'success' ? 'check_circle' : alert.type === 'error' ? 'error' : alert.type === 'warning' ? 'warning' : 'info' .'"></span>',
  '        </div>',
  '        <div class=\"flex-1\">',
  ...
  '  removeAlert(id) {',
  '    if (this.container) {',
  '      const element = this.container.querySelector(".border-l\\[4px\\]:not([style*=\\\"display: none\\\"])");',
  '      if (element) {',
...
[System.IO.File]::WriteAllLines("updateAlert.js", $lines);
Write-Host "updateAlert.js created successfully";