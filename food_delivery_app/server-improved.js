...
const RATE_MAX = 1000; // Increased for development

function rateLimit(req, res, next) {
  // Skip rate limiting for local development
  if (req.ip === '::1' || req.ip === '127.0.0.1' || req.ip.includes('localhost')) {
    return next();
  }
  ...
  if (requests.length >= RATE_MAX) {
    console.log(`⚠️ Rate limit exceeded for IP: ${key}`);
...
async function initFirebase() {
  // Check for Firestore emulator first
  ...
  if (emulatorRunning) {
    try {
      ...
      if (admin.apps.length === 0) {
        ...
        console.log('✅ Firebase connected via Emulator at localhost:8080');
        return true;
      }
    } catch (err) {
      console.log('⚠️ Emulator connection failed:', err.message);
    }
  }
  ...
  try {
    ...
    if (serviceAccountBase64) {
      ...
      if (!serviceAccountPath) {
        ...
        console.log('❌ Firestore emulator not detected at localhost:8080\n');
        console.log('📋 To fix this, choose ONE of these options:\n');
        console.log('Option 1 - Use Firebase Emulator (Recommended for local testing):');
        console.log('   1. Keep Firestore emulator running: firebase emulators:start --only firestore');
        console.log('   2. Restart this server\n');
        console.log('Option 2 - Add service account to .env:');
        console.log('   1. Get your service account from Firebase Console:');
        console.log('      https://console.firebase.google.com/project/fooddelievry-dce15/settings/serviceaccounts');
        console.log('   2. Click "Generate new private key"');
        console.log('   3. Save as "serviceAccountKey.json" in project root');
        console.log('   4. Run: node scripts/setup-firebase.js');
        console.log('   5. Copy output to your .env file\n');
        console.log('Option 3 - Skip Firebase (API will return 503 errors):');
        console.log('   The web UI will still work with Demo Login button\n');
        console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n');
        return false;
      }
    ...
    if (admin.apps.length === 0) {
      admin.initializeApp({
        ...
        projectId: serviceAccount.project_id || 'fooddelievry-dce15'
      });
    }
...
// Root and specific HTML routes (defined BEFORE static middleware to ensure they work)
app.get('/', (req, res) => {
...
app.get('/restaurant', (req, res) => {
  res.sendFile(path.join(__dirname, 'web', 'restaurant.html'));
});

app.get('/track-order', (req, res) => {
  res.sendFile(path.join(__dirname, 'web', 'track-order.html'));
});

app.get('/orders', (req, res) => {
  res.sendFile(path.join(__dirname, 'web', 'orders.html'));
});
...
// Favicon redirect for browsers requesting .ico
...
// These should be AFTER all API routes

// SPA fallback for client-side routing on specific paths
const spaRoutes = ['/app', '/dashboard', '/profile', '/orders', '/cart'];
