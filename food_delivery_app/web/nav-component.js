...
const SmartNav = (() => {
  ...
  const NAV_ITEMS = [
    ...
    { id: 'cart',      label: 'Cart',       icon: 'shopping_cart',  href: '/checkout' },
    { id: 'profile',   label: 'Profile',    icon: 'person',         href: '/profile' },
  ];

  const DRIVER_NAV = [
    { id: 'home',      label: 'Home',      icon: 'home',           href: '/main' },
    { id: 'orders',    label: 'Orders',     icon: 'receipt_long',   href: '/driver' },
    { id: 'earnings',  label: 'Earnings',   icon: 'payments',       href: '/driver' },
    { id: 'profile',   label: 'Profile',    icon: 'person',         href: '/profile' },
  ];
  ...
  const ADMIN_NAV = [
    ...
    { id: 'users',     label: 'Users',      icon: 'group',          href: '/admin' },
    { id: 'profile',   label: 'Profile',    icon: 'person',         href: '/profile' },
  ];

  const CATEGORIES = [
    { name: 'Food',      icon: 'lunch_dining',     color: '#064e3b', href: '/discovery?category=food' },
    { name: 'Dairy',     icon: 'local_drink',       color: '#10b981', href: '/discovery?category=dairy' },
    { name: 'Fruits',    icon: 'nutrition',         color: '#059669', href: '/discovery?category=fruits' },
    { name: 'Groceries', icon: 'shopping_basket',   color: '#064e3b', href: '/discovery?category=groceries' },
    { name: 'Bakery',    icon: 'bakery_dining',     color: '#f59e0b', href: '/discovery?category=bakery' },
  ];
  ...
  function getCartCount() {
    ...
    if (window.CartService && window.CartService.getItemCount) {
      return window.CartService.getItemCount();
    }
    // Fallback with support for both quantity and qty
    try {
      const cart = JSON.parse(localStorage.getItem('smartsoko_cart') || '[]');
      return cart.reduce((sum, item) => sum + (item.quantity || item.qty || 1), 0);
    } catch { return 0; }
  }
  ...
  function renderTopBar() {
    ...
    bar.innerHTML = `
      <style>
        ...
        .sn-search input:focus { border-color: #064e3b; background: #fff; }
        ...
        .sn-btn-ghost:hover { background: #f0fdf4; color: #064e3b; }
        .sn-btn-primary { background: #064e3b; color: #fff; }
        .sn-btn-primary:hover { background: #e55500; }
        .sn-btn-outline { background: #fff; color: #1a1a1a; border: 1px solid #e0e0e0; }
        .sn-btn-outline:hover { border-color: #064e3b; color: #064e3b; }
        .cart-badge-count {
        ...
        .sn-drawer-link:hover { background: #f0fdf4; color: #064e3b; }
        .sn-drawer-link.active { background: #064e3b; color: #fff; }
        ...
        .sn-breadcrumbs a:hover { color: #012d1d; }
        ...
        .sn-bottom-item:hover { color: #012d1d; background: #fff7ed; }
        .sn-bottom-item.active { color: #064e3b; }
      ...
      <div class="sn-topbar-inner">
        <div style="display:flex;align-items:center;gap:0.75rem;">
          ...
          <a href="/main" class="sn-logo" aria-label="SmartSoko Home">
            ...
            <span style="font-size:1.1rem;color:#064e3b;">SmartSoko</span>
          </a>
        </div>
    ...
    if (searchInput) {
      searchInput.addEventListener('keypress', (e) => {
        if (e.key === 'Enter' && searchInput.value.trim()) {
          window.location.href = `/discovery?search=${encodeURIComponent(searchInput.value.trim())}`;
        }
      });
    }
    ...
    overlay.innerHTML = `
      <div class="sn-drawer" id="sn-drawer">
        <div class="sn-drawer-header">
          <a href="/main" class="sn-logo" style="font-size:1.1rem;">
