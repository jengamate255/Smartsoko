...
const MerchantApp = (() => {
  ...
  const utils = {
    getProductImageList(product) {
      ...
      for (const c of candidates) {
        const url = typeof c === 'string' ? c : (c && (c.url || c.downloadURL || c.src));
  ...
  const templates = {
    ...
    productCardImages(item) {
      const urls = utils.getProductImageList(item);
      const primary = urls[0] || 'images/default-product.png';
      if (urls.length <= 1) {
        return `<img src="${utils.escapeHtml(primary)}" alt="${utils.escapeHtml(item.name)}" class="w-full h-full object-cover" onerror="this.onerror=null; this.src='images/default-product.png';">`;
      }
      const slides = urls.map((url, i) =>
        `<img src="${utils.escapeHtml(url)}" alt="" class="carousel-slide w-full h-full object-cover absolute inset-0 ${i === 0 ? '' : 'hidden'}" onerror="this.onerror=null; this.src='images/default-product.png';">`
      ).join('');
      return `${slides}<span class="absolute bottom-1 right-1 text-[10px] bg-black/50 text-white px-1.5 py-0.5 rounded z-10">${urls.length}</span>`;
    },
    ...
    productItem(item) {
      ...
      return `
        <div class="border rounded-lg p-4 hover:shadow-md transition-shadow bg-white flex flex-col h-full relative group">
          ...
          <div class="w-full h-32 rounded-md mb-3 overflow-hidden bg-gray-100 relative product-image-carousel group" data-product-id="${item.id}" data-index="0">
            ${utils.productCardImages(item)}
  ...
  const actions = {
    ...
    openEditMenuItem(productId) {
      ...
      if (typeof window.setMenuItemImages === 'function' && typeof window.getProductImageList === 'function') {
        window.setMenuItemImages(window.getProductImageList(product));
      } else if (elements.menuItemImage) {
    ...
    async submitAddMenuItem(e) {
      ...
      let imageUrl = '';
      let imageUrls = [];
      if (typeof window.collectMenuItemImages === 'function') {
        const collected = window.collectMenuItemImages();
        imageUrl = collected.imageUrl || '';
        imageUrls = collected.imageUrls || [];
      } else {
        const rawImage = elements.menuItemImage?.value || '';
        imageUrl = (rawImage && !rawImage.startsWith('blob:')) ? rawImage.trim() : '';
      ...
      if (typeof window !== 'undefined' && window.menuImageUploadInProgress) {
        actions.showNotification('Please wait for the image upload to finish before saving.', 'error');
        return;
      }
