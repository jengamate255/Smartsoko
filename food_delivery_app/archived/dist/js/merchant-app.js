...
const MerchantApp = (() => {
  ...
  const actions = {
    ...
    logout() {
      localStorage.removeItem('smartsoko_auth_mode');
      localStorage.removeItem('smartsoko_demo_user');
    ...
    async loadOrders() {
      state.allOrders = await api.getOrders(state.currentSeller.id);
      actions.renderOrders();
  ...
  async function init() {
    ...
    if (authMode === 'demo') {
      const demoUser = JSON.parse(localStorage.getItem('smartsoko_demo_user') || '{}');
      if (demoUser.uid && (demoUser.role === 'merchant' || demoUser.role === 'seller')) {
        state.currentSeller = await api.getSeller(demoUser.uid);
